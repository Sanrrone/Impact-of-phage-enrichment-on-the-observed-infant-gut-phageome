...
if(F){
  if(file.exists("hbins_p7_tc7_ac85_cv7.tsv")){
    ...
    hbins<-mclapply(unique(df_rcounts$hepID),function(h){
      #print(h)
      allowed<-df_eval%>%filter(hepID==h)
      tmp<-subset(df_rcounts, hepID==h & ID%in%allowed$ID)
...
if(F){
  df_pair<-lapply(heps, function(h){
    tdf<-tax_df %>% filter(hepID==h, !is.na(family))
    tdf$ID1<-tdf$ID
    tdf$ID<-NULL
    #tmp<-df_idn %>% filter(hepID==h)
    tmp<-atclust_df %>% filter(hepID==h)
    #tmp<-maniac_df %>% filter(hepID==h)
    #tmp<-atclust_df %>% filter(hepID==h)
...
cls_df<-lapply(heps, function(h){
  vtmp<-subset(vbins_df,hepID==h)
  atmp<-subset(cls_df_atclust, hepID==h)
...
###
tax_host_df<-merge(cdf,tax_df%>%filter(!is.na(score)), by=c("hepID","ID"), all.x = T)
tax_host_df<-merge(tax_host_df,df_host, by=c("hepID","ID"), all.x = T)
tax_host_df<-merge(tax_host_df, df_phatyp, by=c("hepID","ID"), all.x=T)
...
if(F){
  tax_host_df<-merge(tax_host_df,hbins,  by=c("hepID","ID"), all.x = T)
  tax_host_df<-lapply(unique(tax_host_df$hepID), function(h){
    #print(h)
    tmp<-subset(tax_host_df, hepID==h)
...
cdftax<-arrange(tibble(lapply(unique(tax_host_df$hepID),function(h){
  allowed<-df_eval%>%filter(hepID==h)
  stmp<-subset(tax_host_df, hepID==h & ID%in%allowed$ID)
  sbin<-subset(cls_df, hepID==h & ID%in%allowed$ID)
  tmp<-merge(stmp, sbin, by=c("hepID","ID"), all.x=T)
...
cdftax<-lapply(unique(cdftax$hepID), function(h){
  #print(h)
  tmp<-subset(cdftax, hepID==h)
